# requires data and scripts from https://github.com/sebschu/adaptation

setwd("~/Dropbox/Uni/RA/adaptation/adaptation/")

plot_path = function(filename) {
 return(paste("~/Dropbox/Uni/Dissertation/doktorarbeit/plots/", filename, sep=""))
}
...
ggsave(pre_test_main, filename = plot_path("fig-3-pre_test_main.pdf"), width = 30, height = 12, units = "cm")
ggsave(pre_test_s1,   filename = plot_path("fig-B1-pre_test_s1.pdf"),   width = 30, height = 36, units = "cm")
ggsave(pre_test_s2,   filename = plot_path("fig-B2-pre_test_s2.pdf"),   width = 30, height = 28, units = "cm")
...
ggsave(pre_test_main_indiv, filename = plot_path("fig-4-pre_test_main_indiv.pdf"), width = 30, height = 12, units = "cm")
...
ggsave(pre_test_main, filename = plot_path("fig-6-pre_test_model_main.pdf"), width = 30, height = 12, units = "cm")
ggsave(pre_test_s1,   filename = plot_path("fig-D1-pre_test_model_s1.pdf"),   width = 30, height = 36, units = "cm")
ggsave(pre_test_s2,   filename = plot_path("fig-D2-pre_test_model_s2.pdf"),   width = 30, height = 28, units = "cm")
...
get_data_for_qud_plot = function(path, condition) {
  d = read.csv(path)
  d = remove_quotes(d)
  d = spread_data(d)
  d = get_data_for_plotting(d)
  d$condition = condition
  return(d)  
}

d.preference = get_data_for_qud_plot("./experiments/4_comprehension_pre_test/data/4_comprehension_pre_test-prod_trials.csv", "preference")
d.dispreference = get_data_for_qud_plot("./experiments/8_pre_test_neg/data/8_pre_test_neg-trials.csv", "dispreference")
d.disjunction = get_data_for_qud_plot("./experiments/9_pre_test_or/data/9_pre_test_or-trials.csv", "disjunction")

d.qud = rbind(d.preference, d.dispreference, d.disjunction)
d.qud$condition = factor(d.qud$condition, levels = c("preference", "dispreference", "disjunction"), ordered = T)

qud.plot = d.qud %>%
  ggplot(aes(x=percentage_blue, y=rating_m, col=condition)) +
  geom_line(size=1) + 
